  ...
  let result =
        ...
        |> list.map(fn(detail) {
          ...
          let amount =
            ...
            |> result.lazy_unwrap(fn() {
              panic as { "Couldn't parse: " <> amount }
            })
          #(compound, amount)
        })
    ...
    |> list.filter(fn(aunt) {
      ...
      list.all(compounds, fn(compound) {
        let value =
          dict.get(detected_compounds, compound.0)
          |> result.lazy_unwrap(fn() {
            panic as { "Couldn't find value for key: " <> compound.0 }
          })

        case compound.0 {
          "cats" | "trees" -> value < compound.1
          "pomeranians" | "goldfish" -> value > compound.1
          _ -> value == compound.1
        }
      })
    })
