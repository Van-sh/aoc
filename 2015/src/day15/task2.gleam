...
fn task2() -> Nil {
  let result =
    ...
    |> list.map(fn(ingredient) {
      let assert [capacity, durability, flavor, texture, calories] =
    ...
    |> list.map(fn(combination) {
      let combination =
        list.fold(combination, Ingredient(0, 0, 0, 0, 0), fn(acc, ingredient) {
          Ingredient(
      ...
      Ingredient(
        ...
        int.max(0, combination.calories),
      )
    })
    |> list.fold(0, fn(max, cookie) {
      case cookie.calories {
        500 -> {
          let cookie =
