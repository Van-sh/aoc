...
fn task1() -> Nil {
  ...
  let result =
    ...
    |> fn(boss_stats) {
      use min_cost, weapon <- list.fold(weapons, 300)
      use min_cost, armor <- list.fold(armors, min_cost)
      use min_cost, selected_rings <- list.fold(rings, min_cost)
      let player_stats =
        list.fold(
          selected_rings,
